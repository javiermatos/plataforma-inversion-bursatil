...
for i = 1:length(pairs)/2
    ...
    for k = 1:length(varargin{2*i})/2
        propertyIndex = [propertyIndex {{index k}}];
    end
...
if searchSpaceDimension == 1
    ...
    set(axesHandle,'XTickLabel',propertyDomain{propertyIndex{1}{1}}{propertyIndex{1}{2}});
    xlabel(axesHandle, ['\bf' '\{' num2str(propertyIndex{1}{1}) '\}.' propertyName{propertyIndex{1}{1}}{propertyIndex{1}{2}}]);
...
%     zlabel(axesHandle, '\bfFitness');
    ...
    set(axesHandle,'YTick',1:propertyDomainSize(2));
    set(axesHandle,'YTickLabel',propertyDomain{propertyIndex{2}{1}}{propertyIndex{2}{2}});
    ylabel(axesHandle, ['\bf' '\{' num2str(propertyIndex{2}{1}) '\}.' propertyName{propertyIndex{2}{1}}{propertyIndex{2}{2}}]);
...
elseif searchSpaceDimension == 3
    ...
    set(axesHandle,'YTickLabel',propertyDomain{propertyIndex{1}{1}}{propertyIndex{1}{2}});
    ylabel(axesHandle, ['\bf' '\{' num2str(propertyIndex{1}{1}) '\}.' propertyName{propertyIndex{1}{1}}{propertyIndex{1}{2}}]);
    
    set(axesHandle,'XTick',1:propertyDomainSize(2));
    set(axesHandle,'XTickLabel',propertyDomain{propertyIndex{2}{1}}{propertyIndex{2}{2}});
    xlabel(axesHandle, ['\bf' '\{' num2str(propertyIndex{2}{1}) '\}.' propertyName{propertyIndex{2}{1}}{propertyIndex{2}{2}}]);
    
    set(axesHandle,'ZTick',1:propertyDomainSize(3));
    set(axesHandle,'ZTickLabel',propertyDomain{propertyIndex{3}{1}}{propertyIndex{3}{2}});
    zlabel(axesHandle, ['\bf' '\{' num2str(propertyIndex{3}{1}) '\}.' propertyName{propertyIndex{3}{1}}{propertyIndex{3}{2}}]);
...
% Axes
currentIndex = 1;
optimumItems = cell(1, searchSpaceDimension);
for i = 1:searchSpaceDimension
    for j = 1:length(propertyName{i})
        optimumItems{currentIndex} = ['\{' num2str(i) '\}.' propertyName{i}{j} ' = ' num2str(innerValue(propertyDomain{i}{j}(bestIndexArray(currentIndex)))) ', '];
...
% Figure
currentIndex = 1;
descriptionItems = cell(1, searchSpaceDimension);
for i = 1:searchSpaceDimension
    for j = 1:length(propertyName{i})
        descriptionItems{currentIndex} = ['{' num2str(i) '}.' propertyName{i}{j} ' (' num2str(length(propertyDomain{i}{j})) '), '];
        currentIndex = currentIndex + 1;
    end
end
